...
public:
	...
	ImageProjection(): nh("~"){
		...
		// Subscribe할 토픽 설정
		subLidar.subscribe(nh, "/os_cloud_node/points", 50);

		// callback
		subLidar.registerCallback(&ImageProjection::gazeboCallback, this);
	...
	void clearMemory(){
		...
		RconeCloud->clear();
		debugCloud->clear();
		carMarkerArray->markers.clear();
		objMarkerArray->markers.clear();
		angleMarkerArray->markers.clear();
	}


    void gazeboCallback(const sensor_msgs::PointCloud2::ConstPtr& rosCloud)
    {
	...
	void getPointCloud(){
	// 해딩
		pointCloudGenerator.getInterestCloud(fullCloud, interestCloud, {-2, 15}, {-15, 15}, {-0.6, 3.5});
	...
	// 클러스터링
		//pointCloudGenerator.getInterestCloud(fullCloud, interestCloud, {-2, 15}, {-8, 1}, {-0.6, 3.5});
		//pointCloudGenerator.getObjectClusterCloud(interestCloud, objMarkerArray, carMarkerArray, debugCloud);
...
{
	ros::init(argc, argv, "practics_dynamic_noGPS");
